...
% A la recherche d'une intro
    ...
    % 2/
    \item "Le domaine de la vision par ordinateur, poussé par les progrès scientifiques et technologiques récents, s’oriente vers l’analyse en temps réel des scènes comportant des sujets humains." (Source: \href{https://tel.archives-ouvertes.fr/tel-00004804/document}{https://tel.archives-ouvertes.fr/tel-00004804/document} ) %- titre / auteur: Vision par ordinateur pour l’interaction homme-machine fortement couplée - De François Bérard - Thèse) \\
    \\
    \end{itemize}
...
L'interprétation dans ce cas est que l'image est en BGR ( Blue Green Red). Donc les données affichézs sont par paquets de 3 valeurs (vecteur de 3). Le premier pixel par exemple a pour valeur [1,1,255], qui vaut la couleur ROUGE.

        \subsection{Ouvrir et afficher une image avec OpenCV}
                Une image peut être enregistrée sur un disque sous format .jpg, .png... Dans ce cas là, une fonction nous permettra de charger l'image. Pour cela nous utiliserons "imread()". Cela dit, via une webcam nous pouvons prendre une photo et la charger. Pour cela, nous utiliserons la fonction "VideoCapture". Deux choix sont possibles: utiliser une image déjà créer ou prendre une photo.


                \subsubsection{"imread"}
                La fonction \textit{imread()} va nous servir pour traiter dans un premier temps une image enregistré sur le disque.
                L'image en question est Lena ou Lenna.
                Cette photo est un standard dans le test d'image pour deux raisons selon David C. Munson, éditeur en chef lors des discussions de l'IEEE sur le traitement d'image de janvier 1996: 
...
int main(int argc, char** argv ){
    ...
    return 0;
                ...
                La fonction \textit{VideoCapture} nous permet de prendre un instantane via une caméra.
                \begin{minted}[
                    baselinestretch=1.4,
                    bgcolor=LightGray,
                    fontsize=\footnotesize]
...
int main (int argc, char **argv){
  ...
  return 0;
               ...
               \end{minted}
               \begin{center}
                   \textit{Prise d'instantané depuis la caméra et affichage}
               \end{center}
        ...
        \subsection{Changement d'espace couleur}
            Le changement d'espace de couleur permet de voir différemment la même image et ainsi la traiter avec plus d'efficacité. Voici deux espaces de couleur différents de BGR, les différentes luminances de gris et les teintes-saturation-valeur (HSV).
...
int main(int argc, char ** argv){
        ...
        return 0;
                ...
                \textit{Résultat 3 - Affichage image d'origine et teintes de gris}
                \end{center}


                \subsubsection{BGR2HSV}
                Pour notre mission de compter des passants, nous avons besoin de différencier un humain d'un arbre ou bâtiment. Dans l'espace de couleur HSV (Hue, Saturation, Value) à la différence d'autres espaces de couleurs, tous les humains ont la même teinte (Hue).
                Pour changer l'espace BGR en HSV, le code est quasi le même que précédent sauf un paramètre dans \textit{cvtColor}:\\
...
int main (int argc, char *argv[]){
    ...
    Le traitement ne permet pas d'avoir un résultat parfait. Pour améliorer le filtre, l'utilisation de dilatation et érosion d'image est une méthode possible.\\
    
    \subsection{Suppression de fond}
    \subsubsection{MOG2}
    
    Sur l'étape précédente, le résultat nous affiche le fond d'une image alors que le résultat devrait être juste la peau humaine. Cela dit, si la couleur est dans la même tranche de valeur que celle demandée l'objet va apparaître sur l'image. 
...
\end{minted}
    ...
    \begin{center}
        ...
        \textit{Suppression du fond et affichage de la couleur originale des pixels}
    \end{center}
...
Le point \textbf{M} à pour coordonnées x,y,z. Que l'on écrit: $$\textbf{M}\begin{bmatrix}x\\y\\z\end{bmatrix}$$
...
La préférence de coordonnées homogènes nous fait écrire le système:
$$\left\{ 
    \begin{array}{ll}
        v'=fY+U0\\ \\
        u'=fX+U0 \\ \\
...
Avoir une deuxième point de vue nous permet de définir \textbf{w}.
 ...
 \subsubsection{Changement de repère}
...
$$ \begin{matrix}
    ...
    \underbrace{\begin{pmatrix}
        X\\Y\\Z\\1\\
    \end{pmatrix}_{\text{R0}}}_{Sur le repère 0}
...
Le principe de la Vision stéréo est de pouvoir retrouver la position d'un objet sur un espace 3D via plusieurs vues de celui-ci.
 \begin{center}
    \begin{tikzpicture}
...
On va définir Caméra 1 par rapport au repère 0.
...
\begin{itemize}
    \item[$\bullet$] Un épipôle est le centre de projection d'une caméra vue dans le plan image de l'autre caméra. Seule une droite épipolaire passe par chaque point des images (sauf aux épipôles)
    \item[$\bullet$] Une droite épipolaire est formé par l'épipôle et un point de l'image (multitude de droites épipôlaires par image)
\end{itemize}
...
Grâce au livre \textit{Learning OpenCV: Computer Vision with the OpenCV Library by Gary Bradski and Adrian Kaehler}, le site web d'OpenCV et leur github, j'ai pu trouver des explications et des programmes pour réaliser la calibration.\\

Le programme de calibration stéréo qui se trouve section \ref{stereo_calib.cpp}, nous permet de calculer les paramètres intrinsèques et extrinsèques.  \\

Pour exécuter ce programme, je dois le compiler. Pour cela, j'ai utilisé un CMakeList.txt (permet d'exécuter un ensemble d'actions). J'ai trouvé cette utilisation plus agréable que la commande g++... (qui est très longue).
...
J'ai accès à mon programme, que je lance avec argument la largeur de mon plateau d'échec et sa hauteur (8x6) ainsi que le nombre d'instantanés que je souhaite (15).
...
Dans un premier temps, le programme va détecter les chessboards (plateau d'échec) pour nous afficher que tout marche. Si rien n'est affiché c'est que nous avons mal renseigner les arguments. \\
...
Tout se passe bien, nous remarquons qu'il détecte bien notre chessboard. Pour afficher ce résultat sur l'image, le programme rentre dans la fonction \textit{calibrateFromeSavedImages()} et utilise une autre fonction: \textit{findChessboardCornersAndDraw()}. Cette dernière fonction a besoin de l'image de gauche et l'image de droite et va les traiter séparément. Pour l'instant aucune stéréo n'est imaginée. La fonction \textit{findChessboardCornersAndDraw()} va modifier l'espace de couleur des images pour aux venir en niveau de gris. Puis utilise la fonction \textit{findChessboardCorners()} qui va lire l'image pour trouver la position intérieure des coins du chessboard et afficher des ronds liés avec la fonction \textit{drawChessboardCorners()}. Dès lors, les images sont affichées à l'écran.\\
